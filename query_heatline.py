...
# Get sessions from the last 30 days for heatline project
...
cursor.execute("""
    ...
    FROM session
    WHERE time_created > ?
      AND directory LIKE '%heatline%'
    ORDER BY time_created DESC
""", (thirty_days_ago,))

sessions = cursor.fetchall()
print(f"Heatline sessions in last 30 days: {len(sessions)}")
for s in sessions:
    title = s[1] or "Untitled"
    print(f"  {s[0]}: {title}")

# Get tool usage patterns for heatline sessions
