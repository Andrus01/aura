...
# Get sessions from the last 30 days for toukeratas project
...
cursor.execute("""
    ...
    FROM session
    WHERE time_created > ?
      AND directory LIKE '%toukeratas%'
    ORDER BY time_created DESC
""", (thirty_days_ago,))

sessions = cursor.fetchall()
print(f"toukeratas sessions in last 30 days: {len(sessions)}")
for s in sessions:
    title = s[1] or "Untitled"
    print(f"  {s[0]}: {title}")

# Get tool usage patterns for toukeratas sessions
...
if session_ids:
    ...
    print("\ntoukeratas tool usage patterns:")
    for r in results:
        tool = r[0] or "unknown"
        preview = (r[1] or "")[:150]
        count = r[2]
